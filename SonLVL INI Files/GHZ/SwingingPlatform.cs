...
namespace S1ObjectDefinitions.GHZ
{
	class SwingingPlatform : ObjectDefinition
	{
		...
		{
			imgwreckingball = ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../res/Art/GHZBall", CompressionType.Nemesis), "../asm/Mappings/GHZBall.asm", 1, 2);
			for (int i = 0; i < labels.Length; i++)
				imgs.Add(ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../res/Art/GHZSwing", CompressionType.Nemesis), "../asm/Mappings/GHZSwing.asm", labels[i], i == 1 ? 0 : 2));
		}
